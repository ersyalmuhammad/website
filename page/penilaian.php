...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($update) {
		...
		$sql = "INSERT INTO penilaian VALUES (NULL, '$_POST[kd_jabatan]', '$_POST[kd_kriteria]', '$_POST[keterangan]', '$_POST[bobot]')";
		$validasi = true;
	}

	if ($validasi) {
		$q = $connection->query("SELECT kd_penilaian FROM penilaian WHERE kd_jabatan=$_POST[kd_jabatan] AND kd_kriteria=$_POST[kd_kriteria] AND keterangan LIKE '%$_POST[keterangan]%' AND bobot=$_POST[bobot]");
		if ($q->num_rows) {
	            ...
	            <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST">
	                  ...
	                  <label for="kd_kriteria">Kriteria</label>
										<select class="form-control" name="kd_kriteria" id="kriteria">
											...
											<?php while ($data = $sql->fetch_assoc()): ?>
												<option value="<?=$data["kd_kriteria"]?>" class="<?=$data["kd_jabatan"]?>" <?= (!$update) ?: (($row["kd_kriteria"] != $data["kd_kriteria"]) ?: 'selected="selected"') ?>><?=$data["nama"]?></option>
											<?php endwhile; ?>
										</select>
									</div>
	                <div class="form-group">
	                    <label for="keterangan">Keterangan Sub kriteria</label>
	    ...
	    <div class="panel panel-info">
	        ...
	        <div class="panel-body">
	            <table class="table table-condensed">
	                ...
	                <tbody>
	                    <?php $no = 1; ?>
	                    <?php if ($query = $connection->query("SELECT a.kd_penilaian, c.nama AS nama_jabatan, b.nama AS nama_kriteria, a.keterangan, a.bobot FROM penilaian a JOIN kriteria b ON a.kd_kriteria=b.kd_kriteria JOIN jabatan c ON a.kd_jabatan=c.kd_jabatan")): ?>
