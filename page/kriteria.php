...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($update) {
		$sql = "UPDATE kriteria SET kd_jabatan=$_POST[kd_jabatan], nama='$_POST[nama]', sifat='$_POST[sifat]' WHERE kd_kriteria='$_GET[key]'";
	} else {
		$sql = "INSERT INTO kriteria VALUES (NULL, $_POST[kd_jabatan], '$_POST[nama]', '$_POST[sifat]')";
		$validasi = true;
	}

	if ($validasi) {
		$q = $connection->query("SELECT kd_kriteria FROM kriteria WHERE kd_jabatan=$_POST[kd_jabatan] AND nama LIKE '%$_POST[nama]%'");
		if ($q->num_rows) {
	            ...
	            <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST">
									<div class="form-group">
										...
										<select class="form-control" name="kd_jabatan">
											<option>---</option>
											<?php $query = $connection->query("SELECT * FROM jabatan"); while ($data = $query->fetch_assoc()): ?>
	    ...
	    <div class="panel panel-info">
	        ...
	        <div class="panel-body">
	            <table class="table table-condensed">
	                ...
	                <tbody>
	                    <?php $no = 1; ?>
	                    <?php if ($query = $connection->query("SELECT a.nama AS kriteria, b.nama AS jabatan, a.kd_kriteria, a.sifat FROM kriteria a JOIN jabatan b USING(kd_jabatan)")): ?>
